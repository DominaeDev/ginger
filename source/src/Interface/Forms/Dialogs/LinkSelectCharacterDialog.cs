...
namespace Ginger
{
	...
	public partial class LinkSelectCharacterDialog : FormEx
	{
		...
		{
			if (Backyard.ConnectionEstablished && Backyard.Current.GetChatCounts(out _chatCounts) != Backyard.Error.NoError)
		...
		private void PopulateTree(bool bRefresh)
		{
			...
			// Create characters
			IEnumerable<CharacterInstance> sortedCharacters = Characters;
			...
			{
				sortedCharacters = sortedCharacters
					.OrderBy(c => c.folderSortPosition);
			}
