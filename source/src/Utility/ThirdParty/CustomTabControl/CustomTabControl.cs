...
namespace System.Windows.Forms
{
	...
	public class CustomTabControl : TabControl, IThemedControl
	{
		...
		#endregion Basic drawing methods
...
//			path.CloseFigure();
		...
		private void AddPageBorder(GraphicsPath path, Rectangle pageBounds, Rectangle tabBounds)
		{
			switch (Alignment)
			{
				...
				case TabAlignment.Right:
					path.AddLine(pageBounds.Right, tabBounds.Bottom, pageBounds.Right, pageBounds.Bottom - 1);
					//path.AddLine(pageBounds.Right, pageBounds.Bottom, pageBounds.X, pageBounds.Bottom);
					//path.AddLine(pageBounds.X, pageBounds.Bottom, pageBounds.X, pageBounds.Y);
					//path.AddLine(pageBounds.X, pageBounds.Y, pageBounds.Right, pageBounds.Y);
		...
		protected override void OnKeyDown(KeyEventArgs e)
		{
			if (e.KeyData == (Keys.Control | Keys.PageDown)
				|| e.KeyData == (Keys.Control | Keys.PageUp)) 
			{
				// Eat keyboard shortcuts
				e.Handled = true;
				return;
			}
			base.OnKeyDown(e);
