...
namespace LlamaTokenizer
{
	public static class TokenizerTests
	{
		...
		private static bool isEqual(ushort[] arr1, ushort[] arr2)
		{
		...
		private static bool testCase(string inputString, params ushort[] expectedTokenIds)
		{
			var actualTokens = LlamaTokenizer.encode(inputString, true, true);

			if (isEqual(actualTokens, expectedTokenIds) == false)
				throw new Exception(string.Format("Encode test failed: {0}", inputString));
			if (inputString != LlamaTokenizer.decode(actualTokens))
				throw new Exception(string.Format("Decode test failed: {0}", inputString));
			return true;
		}
	}
}
